...
export default async function AdminAuditLogsPage() {
  ...
  return (
    <div className="p-8 max-w-6xl">
      <div className="mb-8">
        <h1 className="text-2xl font-serif text-neutral-950 dark:text-neutral-50">Audit logs</h1>
          ...
          <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
            {(logs ?? []).map((log) => (
              <tr key={log.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800">
                <td className="px-5 py-3 text-neutral-600 dark:text-neutral-400 text-xs">{log.org_id ? (orgMap[log.org_id] ?? log.org_id.slice(0, 8) + "…") : "—"}</td>
                <td className="px-5 py-3 font-mono text-xs text-neutral-950 dark:text-neutral-50">{log.action}</td>
