...
export default async function AdminOverviewPage() {
  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="mb-8">
        <h1 className="text-2xl font-serif text-neutral-950">Admin overview</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {[
          ...
          <div key={label} className="bg-white border border-neutral-200 rounded-2xl p-5">
            ...
            <p className="text-3xl font-serif text-neutral-950">{value}</p>
          </div>
        ))}
      </div>
        ...
        {Object.keys(planBreakdown).length === 0 ? (
          ...
          <div className="space-y-2">
            {Object.entries(planBreakdown).map(([plan, count]) => (
              <div key={plan} className="flex items-center justify-between py-1.5 border-b border-neutral-100 last:border-0">
                <span className="text-sm text-neutral-950 capitalize">{plan}</span>
