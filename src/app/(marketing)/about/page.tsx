    ...
    <div>
          ...
          <div className="mt-14 grid gap-4 sm:grid-cols-2">
            {BELIEFS.map((b) => (
              <div key={b.heading} className={`${CARD} p-8`}>
                ...
                <p className="mt-3 text-lg text-neutral-200 leading-relaxed">{b.body}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {STATS.map((item) => (
              <div key={item.label} className={`${CARD} p-8 text-center`}>
                ...
                <p className="mt-3 text-neutral-400">{item.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-neutral-900 py-24 text-center">
        ...
        <div className="mt-9 flex items-center justify-center gap-5">
          ...
          <Link href="/contact" className="text-base text-neutral-200 hover:text-neutral-100 transition-colors">
            Get in touch →
          </Link>
        </div>
      </section>
