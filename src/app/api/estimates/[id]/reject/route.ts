...
export async function POST(_req: NextRequest, { params }: Params) {
  ...
  if (!["sent", "draft"].includes(estimate.status)) {
    return NextResponse.json({ error: "Estimate cannot be rejected in its current state" }, { status: 400 });
  }
  ...
  if (ownerId && org) {
    ...
    if (ownerEmail) {
      ...
      await sendTransactionalEmail({
        ...
        to: ownerEmail,
        subject: `Estimate ${estimate.estimate_number} declined by ${client?.name ?? "client"}`,
