...
export async function POST(_req: NextRequest, { params }: Params) {
  ...
  const approveClient = Array.isArray(estimate.clients) ? estimate.clients[0] : estimate.clients;
  runAutomations(estimate.org_id, "estimate.approved", {
    estimate_id: estimate.id,
    estimate_number: estimate.estimate_number,
    client_name: approveClient?.name ?? undefined,
    amount: estimate.total,
    currency: estimate.currency,
  }).catch(() => {});

  dispatchWebhook(estimate.org_id, "estimate.approved", {
    id: estimate.id,
    estimate_number: estimate.estimate_number,
