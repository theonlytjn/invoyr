...
export async function POST(
  ...
  const { data: emailData, error: emailErr } = await getResend().emails.send({
    ...
    to: client.email,
    subject: `Invoice ${invoice.invoice_number} from ${process.env.NEXT_PUBLIC_ORG_NAME ?? ""}`,
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:40px 20px;">
        ...
        ${payUrl ? `<a href="${payUrl}" style="display:inline-block;margin-top:24px;padding:12px 24px;background:#111827;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;">Pay now</a>` : ""}
