...
- **After a client is deleted, the send and remind routes find no client and skip that invoice — checkout does not.** `src/app/api/invoices/[id]/send/route.ts`, `.../bulk/send`, `.../[id]/remind`, `.../bulk/remind` and `src/app/api/estimates/[id]/send/route.ts` all join `clients(*)` directly (no snapshot fallback) and skip/decline when the join comes back empty, because they need a live email address to send to. This is correct — a deleted client should not receive email, and a snapshot has no inbox — but it will read as a regression to anyone who deletes a client and then notices reminders stop, so it's recorded here rather than left to be filed as a bug. `src/app/api/payments/checkout/route.ts` behaves differently: it never checks whether the join returned a client, and creates the Stripe Checkout session regardless, passing `customer_email: client?.email ?? undefined`. So after a client is deleted, checkout still works — the customer simply doesn't get their email prefilled on the Stripe page. A reader who deletes a client and then wonders why payment links keep working should find the answer here.
